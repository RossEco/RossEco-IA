# ROSSECO AI — Chatbot LLM Free Tier Mistral Évolué

**ROSSECO AI** est une plateforme d'intelligence artificielle personnelle avancée, entièrement contenue dans **une seule page HTML**. Elle exploite la puissance des modèles **Mistral AI** via leur **Free Tier** généreux, offrant des capacités de chatbot sophistiquées avec mémoire globale, agents spécialisés auto-générés, et gestion complète des données — le tout en local, sans serveur backend.

\---

## 🌟 Pourquoi ROSSECO AI est un Chatbot LLM Évolué ?

Contrairement aux chatbots basiques, ROSSECO AI intègre des fonctionnalités avancées directement dans son code source :
...
3. **Système de Mémoire Globale Persistante**
   ...
   * Base de données IndexedDB (`ROSSECO\_AI\_DB`) avec 4 stores : `chats`, `agents`, `global\_memory`, `settings`
...
4. Donnez un nom à votre clé (ex: `ROSSECO-Personal`)
5. **Copiez immédiatement la clé** — elle ne sera affichée qu'une seule fois !

   * Format : `votre\_clé\_commence\_par\_` suivi de caractères alphanumériques
...
La **Mémoire Globale** est un système révolutionnaire qui dépasse le contexte limité des conversations. Contrairement à la mémoire temporaire d'un chat classique (qui disparaît après chaque session), la mémoire globale de ROSSECO AI :
...
const memory = {
  add: async (content, tags = \[]) => {
    ...
    // Stocke dans IndexedDB (store: 'global\_memory')
...
Un **Agent** dans ROSSECO AI n'est pas un simple preset de prompt. C'est une **entité autonome** avec :
...
4. Ou sélectionnez-le via le dropdown **◈ NOM\_AGENT** dans le header
...
ROSSECO AI utilise **IndexedDB** avec 4 stores :

```javascript
const DB\_NAME = "ROSSECO\_AI\_DB";
const DB\_VERSION = 3;
...
// 3. 'global\_memory' : { id, content, tags\[], created, importance }
...
window.memoryDelete = async id => {
  await db.delete('global\_memory', id);
...
async function exportData() {
  ...
  const mems = await db.getAll('global\_memory') || \[];
  ...
  const payload = {
    ...
    source: "ROSSECO",
    data: { chats, agents, global\_memory: mems, settings }
  };
  ...
  a.href = url;
  a.download = `rosseco-backup-${new Date().toISOString().slice(0,10)}.rosseco.json`;
  a.click();
...
3. Téléchargement automatique : `rosseco-backup-2025-01-15.rosseco.json`
...
```json
{
  ...
  "data": {
    ...
    "global\_memory": \[...], // Toute votre mémoire globale
...
async function importData(file) {
  ...
  if (data.global\_memory?.length) {
    for (const m of data.global\_memory) { await db.put('global\_memory', m); count++; }
  }
...
3. Sélectionnez un fichier `.rosseco.json` ou `.json`
...
1. **Sur l'ancien PC** :
   ...
   * Ouvrez ROSSECO AI
   * Archives → ⬇ EXPORTER
   * Récupérez `rosseco-backup-2025-01-15.rosseco.json`
   * Transférez-le (email, cloud, clé USB)
2. **Sur le nouveau PC** :

   * Ouvrez ROSSECO AI (nouvelle installation)
...
**ROSSECO AI** — *Votre Intelligence Artificielle Personnelle, Évoluée, Locale et Gratuite.*
